...
with col2:
    st.metric("Cities", df["scraped_city"].nunique())

with col3:
    st.metric("Work Modes", df["work_mode"].nunique())
...
city = df["scraped_city"].value_counts().head(10).reset_index()
...
workmode = df["work_mode"].value_counts().reset_index()
...
fig5 = px.treemap(df,path=["scraped_city", "role_category"],title="Jobs by City and Role Category"
)
